...
vec3 point_light_color(vec3 pos, int i) {
	...
	// Position of the light in <gubo.lightPos[i]>
	return pow(gubo.lightColor[i].a/length(gubo.lightPos[i] - pos),2) * gubo.lightColor[i].rgb;
}
...
vec3 spot_light_color(vec3 pos, int i) {
	...
	return point_light_color(pos,i) * clamp((normalize(gubo.lightPos[i] - pos)*gubo.lightDir[i] - gubo.cosOut)/(gubo.cosIn - gubo.cosOut),0,1);
}
