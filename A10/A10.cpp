...
struct EarthParameterBufferObject{
	alignas(4) float Pow;
	alignas(4) float Ang;
...
// **A10** Place here the initialization of the the DescriptorSetLayout
        DSLearth.init(this,{
                    {0,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,VK_SHADER_STAGE_VERTEX_BIT, sizeof(EarthUniformBufferObject),1},
                    {1,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,VK_SHADER_STAGE_FRAGMENT_BIT, 0,1},
                    {2,VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,VK_SHADER_STAGE_FRAGMENT_BIT, 1,1},
                    ...
                    {6,VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(EarthParameterBufferObject),1}
        });
...
// **A10** Place here the initialization for the VertexDescriptor
        VDearth.init(this, {
            ...
            {0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(EarthVertex, pos),
                                               sizeof(glm::vec3), POSITION},
            {0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(EarthVertex, normal),
                                               sizeof(glm::vec3), NORMAL},
            {0, 2, VK_FORMAT_R32G32_SFLOAT, offsetof(EarthVertex, UV),
                                               sizeof(glm::vec2), UV},
            {0, 3, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(EarthVertex, tangent),
...
// **A10** Place here the loading of the five textures
        ...
        TnormalMap.init(this,"textures/2k_earth_normal_map.png",VK_FORMAT_R8G8B8A8_UNORM);
...
// **A10** Update the number of elements to correctly size the descriptor sets pool
		DPSZs.uniformBlocksInPool = 7; // 7
		DPSZs.texturesInPool = 9; // 9
		DPSZs.setsInPool = 6; // 6
...
// Textures should be passed in the diffuse, specular, normal map, emission and clouds order.
        ...
        DSearth.init(this,&DSLearth, {&Tdiffuse,&Tspecular,&TnormalMap,&Temission,&Tclouds});
...
// **A10** Add to compute the uniforms and pass them to the shaders. You need two uniforms: one for the matrices, and the other for the material parameters.
        ...
        earthUbo.mMat = glm::mat4(1.0f);
        earthUbo.nMat = glm::mat4(1.0f);
        earthUbo.mvpMat = ViewPrj ;
        DSearth.map(currentImage,&earthUbo,0);

        EarthParameterBufferObject earthParamUbo{};
        earthParamUbo.Pow = 200.0f;
        earthParamUbo.Ang = tTime * TangTurnTimeFact;
        earthParamUbo.ShowCloud = ShowCloud;
        earthParamUbo.ShowTexture = ShowTexture;
        DSearth.map(currentImage,&earthParamUbo,6);
	}
};
